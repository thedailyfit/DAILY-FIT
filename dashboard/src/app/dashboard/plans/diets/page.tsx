...
import { Apple, Plus, Search, Users, MoreVertical, Edit, Copy, Trash, Wand2 } from "lucide-react"
...
export default function DietPlansLibraryPage() {
    ...
    const fetchPlans = async () => {
        ...
        if (!error && data) {
            const mapped = data.map((plan: any) => ({
                id: plan.id,
                title: plan.name,
                macros: `${plan.protein || 0}g P / ${plan.total_calories || 0} kcal`,
                clientsActive: 0,
    ...
    const handleDuplicate = async (plan: any) => {
        ...
        const { error } = await supabase.from('diet_plans').insert([newPlan])
        if (!error) {
            fetchPlans()
        } else {
        ...
        <div className="p-4 md:p-8 space-y-6 max-w-[1600px] mx-auto min-h-screen bg-background text-foreground">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button 
                        variant="secondary"
                        onClick={async () => {
                            const title = prompt("What kind of diet plan do you want to generate (e.g. 'High Protein Vegetarian')?");
                            if (title) {
                                const newPlan = {
                                    name: title,
                                ...
                                await supabase.from('diet_plans').insert([newPlan]);
                                fetchPlans();
                            }
                        }}
                    >
            ...
            <div className="flex items-center gap-2 border-b pb-2">
                {['all', 'active', 'draft', 'archived'].map((status) => (
                    <Button 
                        ...
                        className={statusFilter === status ? "bg-green-600 hover:bg-green-700 text-white capitalize font-bold" : "capitalize"}
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 text-center border rounded-2xl bg-card border-dashed">
                    ...
                    <h3 className="text-xl font-bold mb-2">No Diet Plans Found</h3>
                    <p className="text-muted-foreground max-w-md mb-6">Create nutrition templates to quickly assign to your clients.</p>
                    <Link href="/dashboard/plans/diets/new">
                        <Button className="font-bold bg-green-600 hover:bg-green-700 text-white"><Plus className="h-4 w-4 mr-2" /> Create Plan</Button>
                    </Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 gap-8">
                    {filteredPlans.map((plan: any, index: number) => (
                        <motion.div
                            ...
                            className="h-full"
                        >
                            <Card className="group hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 border-border bg-gradient-to-br from-card to-green-500/5 overflow-hidden flex flex-col rounded-2xl h-full min-h-[400px]">
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <CardDescription className="mt-1 flex items-center gap-2 flex-wrap">
                                                ...
                                                <span className="font-mono text-xs font-semibold bg-green-100 text-green-800 px-2 py-0.5 rounded-full dark:bg-green-900 dark:text-green-200">{plan.macros}</span>
                                            </CardDescription>
                                        </div>
                                ...
                                <CardFooter className="p-4 border-t bg-muted/10 gap-2">
                                    <div className="flex-1">
                                        <AssignToClientDialog planId={plan.id} planName={plan.title} planType="diet" />
                                    </div>
                                    <Link href={`/dashboard/plans/diets/${plan.id}`}>
