...
import { Calendar, Weight, Activity, FileText } from "lucide-react"
...
export function CheckInTimeline({ entries }: CheckInTimelineProps) {
    ...
    if (sortedEntries.length === 0) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle className="text-sm font-medium">Check-in History</CardTitle>
                </CardHeader>
                <CardContent>
                        ...
                        {sortedEntries.map((entry, index) => (
                            <div key={entry.id} className="relative pl-6">
                                ...
                                <div className="flex flex-col gap-2">
                                            ...
                                            <div className="flex items-center gap-2 text-muted-foreground col-span-2">
                                                ...
                                                <div className="flex gap-0.5">
                                                    {Array.from({ length: 10 }).map((_, i) => (
                                                        <div
                                                            key={i}
                                                            className={`h-1.5 w-3 rounded-full ${i < entry.adherence_score!
                                                                    ? "bg-green-500"
                                                                    : "bg-muted"
                                                                }`}
                                                        />
                                    ...
                                    {/* Notes */}
                                    {entry.notes && (
                                        <div className="mt-1 bg-muted/50 p-3 rounded-md text-xs text-muted-foreground italic">
                                            "{entry.notes}"
                                        </div>
                                    )}
